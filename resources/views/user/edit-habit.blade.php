...
@section('content')
    ...
    <div class="max-w-xl mx-auto">
        <div class="bg-white rounded-2xl border border-[#E4DFF5] p-8">
            ...
            <h1 class="font-['Fraunces',serif] font-semibold text-2xl">Modifier l'habitude</h1>
            <p class="text-sm text-[#8B7BAE] mt-1 mb-7">Mettez à jour les informations de cette habitude.</p>

            <form action="{{route('update-habit', $habit->id)}}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" value="{{$habit->nom}}" class="w-full pl-11 pr-4 py-3 rounded-xl bg-[#F8F7FC] border border-[#E4DFF5] text-sm placeholder:text-[#8B7BAE] focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/30 focus:border-[#7C3AED]">
                    </div>
                </div>
